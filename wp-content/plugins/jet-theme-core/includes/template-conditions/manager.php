...
class Manager {
	...
	public function register_cpt_conditions() {
		...
		foreach ( $post_types as $type ) {
			...
			$single_options = \Jet_Theme_Core\Utils::get_posts_by_type( $post_type_slug, '' );
			...
			foreach ( $post_type_taxonomies as $taxonomy => $taxonomy_obj ) {
				$terms_options = \Jet_Theme_Core\Utils::get_terms_options_by_taxonomy( $taxonomy_obj->name );
	...
	public function find_matched_conditions( $type, $single = false ) {
		...
		foreach ( $conditions[ $type ] as $template_id => $template_conditions ) {
			...
			$template_id = apply_filters( 'jet-theme-core/template-conditions/template-id', $template_id );
	...
	public function post_conditions_verbose( $post_id = null ) {
		...
		if ( ! $structure->has_conditions() ) {
			...
			return sprintf(
				...
				$warning_icon,
				__( 'Conditions are not available for this type', 'jet-theme-core' )
			);
		}
