...
class RRMail
{
    ...
    {
        // email notification
        add_action('rr_user_email_notification', array($this, 'send_user_email_notification_action'), 10, 1);
        add_action('rr_admin_email_notification', array($this, 'send_admin_email_notification_action'), 10, 2);

        // we want to check if it is link from EA mail
        add_action('wp', array($this, 'parse_mail_link'));

        // we need to pars
        add_filter('rr_format_notification_params', array($this, 'format_data'), 100, 2);

        // wrap email template into html
        add_filter('rr_admin_mail_template', array($this, 'wrap_email_with_html_tags'), 10, 1);
        add_filter('rr_customer_mail_template', array($this, 'wrap_email_with_html_tags'), 10, 1);
...
EOT;
    ...
    /**
     * FILTER: rr_format_notification_params
     * ORDER: 100
     *
    ...
    public function parse_mail_link()
    {
        ...
        if (empty($_GET['_rr-action']) || empty($_GET['_rr-app']) || empty($_GET['_rr-t'])) {
            return;
        }
        ...
        if (empty($_POST['confirmed']) && (!empty($_POST['confirmed']) && $_POST['confirmed'] !== 'true')) {
            $this->link_action_additional_step($_GET['_rr-action'], $data);
        }

        if (empty($data)) {
            header('Refresh:3; url=' . get_home_url());
            wp_die(__('No appointment.', 'range-reserver'));
        }

        // invalid token
        if ($this->generate_token($data, $_GET['_rr-action']) != $_GET['_rr-t']) {
            header('Refresh:3; url=' . get_home_url());
            wp_die(__('Invalid token.', 'range-reserver'));
        }
        ...
        // confirm appointment
        if ($_GET['_rr-action'] == 'confirm') {

            if ($data['status'] === 'confirm') {
                header('Refresh:3; url=' . get_home_url());
                wp_die(__('Appointment is already confirmed!', 'range-reserver'));
            }
            ...
            if ($data['status'] !== 'pending' && $default_status !== 'reservation' && $data['status'] !== 'reservation') {
                header('Refresh:3; url=' . get_home_url());
                wp_die(__('Appointment can\'t be confirmed!', 'range-reserver'));
            }
            ...
            do_action('rr_new_app', $app_data['id'], $app_data);

            // for user
            do_action('rr_user_email_notification', $app_data['id']);

            // for admin
            do_action('rr_admin_email_notification', $app_data['id']);

            $url = apply_filters( 'rr_confirmed_redirect_url', get_home_url());

            header('Refresh:3; url=' . $url);
            wp_die(__('Appointment has been confirmed.', 'range-reserver'));
        }

        // cancel appointment
        if ($_GET['_rr-action'] == 'cancel') {
            $app_data['status'] = 'canceled';

            if ($data['status'] === 'canceled') {
                wp_die(__('Appointment is already cancelled!', 'range-reserver'));
            }

            // only pending and confirmed appointments can be canceled
            if ($data['status'] != 'pending' && $data['status'] != 'confirmed') {
                wp_die(__('Appointment can\'t be cancelled!', 'range-reserver'));
            }
            ...
            if (new DateTime() > new DateTime($app_data['date'] . ' ' . $app_data['start'])) {
                $url = apply_filters( 'rr_cant_be_canceled_redirect_url', get_home_url());

                header('Refresh:3; url=' . $url);
                wp_die(__('Appointment can\'t be cancelled', 'range-reserver'));
            }

            $url = apply_filters( 'rr_cancel_redirect_url', get_home_url());
            ...
            header('Refresh:3; url=' . $url);
            wp_die(__('Appointment has been cancelled', 'range-reserver'));
        }
    }
    ...
    private function link_action_additional_step($action, $data)
    {
        ...
        $title = __('Appointment link action', 'range-reserver');
     ...
     *
     * @param  int $app_id Application id
     * @param bool $lane_only
     */
    public function send_admin_email_notification_action($app_id, $lane_only = false)
    {
        $this->send_notification(array('id' => $app_id), $lane_only);
    }
     ...
     * @param bool $lane_only
     */
    public function send_notification($input_data, $lane_only = false)
    {
        $emails = array();
        ...
        // lane email
        if ($this->options->get_option_value('send.lane.email', '0') == '1') {
            // if we only want to send it to lane
            if ($lane_only) {
                $emails = array();
            }

            $emails[] = $raw_data['lane_email'];
        }
        ...
        $links['#link_cancel#'] = $this->generate_link_element($raw_data, 'cancel', __('Cancel appointment', 'range-reserver'));
        $links['#link_confirm#'] = $this->generate_link_element($raw_data, 'confirm', __('Confirm appointment', 'range-reserver'));
        ...
        $emails = apply_filters('rr_admin_mail_address_list', $emails, $raw_data);

        $body_template = $this->options->get_option_value('mail.admin', '');

        $body_template = apply_filters('rr_admin_mail_template', $body_template);
        ...
        if (!empty($body_template)) {
            ...
            require RR_SRC_DIR . 'templates/mail.notification.tpl.php';
        ...
        $files = apply_filters('rr_admin_mail_attachments', $files, $raw_data);
    ...
    public function send_status_change_mail($app_id)
    {
        ...
        $subject_template = apply_filters( 'rr_customer_mail_subject_template', $subject_template);

        $body_template = $this->options->get_option_value('mail.' . $app->status, 'mail');

        // Hook for customize body of email template
        $body_template = apply_filters( 'rr_customer_mail_template', $body_template, $app_array, $params );
        ...
        if (count($email_to) > 0) {
            ...
            $files = apply_filters('rr_user_mail_attachments', $files, $app_array);
    ...
    {
        $table_name = $this->wpdb->prefix . 'rr_error_logs';
