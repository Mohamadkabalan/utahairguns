...
class tad_DI52_Container implements ArrayAccess {
	...
	protected function resolve($classOrInterface) {
		...
		try {
			if (isset($this->deferred[$classOrInterface])) {
				/** @var tad_DI52_BayProviderInterface $provider */
	...
	/**
	 * Registers a bay provider implementation.
	 *
	 * The `register` method will be called immediately on the bay provider.
	 *
	 ...
	 * container itself.
	 *
	 * @see tad_DI52_BayProviderInterface::register()
	 * @see tad_DI52_BayProviderInterface::isDeferred()
	 * @see tad_DI52_BayProviderInterface::provides()
	 * @see tad_DI52_BayProviderInterface::boot()
	 *
	 * @param string $bayProviderClass
	 */
	public function register($bayProviderClass) {
		/** @var tad_DI52_BayProviderInterface $provider */
		$provider = new $bayProviderClass($this);
		if (!$provider->isDeferred()) {
			...
			if ($count === 0) {
				throw new RuntimeException("Bay provider '{$bayProviderClass}' is marked as deferred but is not providing any implementation.");
			}
	...
	/**
	 * Boots up the application calling the `boot` method of each registered bay provider.
	 *
	 ...
	 *      // but if the requesting class is `Lane` return another implementation
	 *      $container->when('Lane')
