A PHP 5.2 compatible dependency injection container heavily inspired by [Laravel IOC](https://laravel.com/docs/5.0/container "Bay Container - Laravel - The PHP Framework For Web Artisans") and [Pimple](https://github.com/silexphp/Pimple "Pimple - A simple PHP Dependency Injection Container") that works even better on newer version of PHP.
...
class ClassOne implements InterfaceOne {}
 ...
 // MysqlSchedule.php
class MysqlSchedule implements DbScheduleInterface {}

 // ClassThree.php
class ClassThree {
    public function __construct(InterfaceOne $one, DbScheduleInterface $db){
       // ... 
    }
}
...
$container->singleton('DbScheduleInterface', 'MysqlSchedule');
...
Once an instance of di52 is available telling it what should be built and when is quite easy; di52 proposes the same API exposed by [Laravel Bay container](https://laravel.com/docs/5.3/container "Bay Container - Laravel - The PHP Framework For Web ...") and while the inner workings are different the good idea (Laravel's) is reused.  
...
$container->bind('DBInterface', function(tad_DI52_Container $container){
    ...
    return new DBSchedule($dbDriver);
});
```
...
$container['db.schedule'] = function($container){
    $dbDriver = $container->make('db.driver');

    return new DBSchedule($dbDriver);
};

$dbSchedule = $container['db.schedule'];
...
## Bay providers
To avoid passing the container instance around (see [Bay Locator pattern](https://en.wikipedia.org/wiki/Bay_locator_pattern "Bay locator pattern - Wikipedia")) or globalising it all the binding should happen in the same PHP file: this could lead, as the application grows, to a thousand lines monster.  
To avoid that the container supports bay providers: those are classes implmenting the `tad_DI52_BayProviderInterface` interface, or extend the ready to use `tad_DI52_BayProvider` class, that allow organizing the binding registrations into logical, self-contained and manageable units:

```php
// file ProviderOne.php
class ProviderOne extends tad_DI52_BayProvider {
...
### Booting bay providers
The container implements a `boot` method that will, in turn, call the `boot` method on any bay provider that overloads it.  
...
In that case bay providers can overload the `boot` method:
...
To "defer" that cost bay providers can overload the `deferred` property and the `provides` method:
