    ...
    jQuery.extend(Plugin.prototype, {
        ...
        init: function () {
            ...
            this.$element.find('.time').on('click', '.time-value', function (event) {
                ...
                if (!result) {
                    alert(rr_settings['trans.slot-not-selectable']);
                    return;
                }
                ...
                if (rr_settings['pre.reservation'] === '1') {
                    ...
                    booking_data.date_time = moment(booking_data.date + ' ' + booking_data.time, rr_settings['default_datetime_format']).format(format);
                    ...
                    jQuery(document).trigger('rr-timeslot:selected');
                }
            });
        ...
        callServer: function (options, next_element) {
            ...
            jQuery.get(rr_ajaxurl, options, function (response) {
                ...
                jQuery.each(response, function (index, element) {
                    ...
                    if ('price' in element && rr_settings['price.hide'] !== '1') {

                        if (rr_settings['currency.before'] == '1') {
        ...
        appSelected: function (element) {
            ...
            var format = rr_settings['date_format'] + ' ' + rr_settings['time_format'];
            booking_data.date_time = moment(booking_data.date + 'T' + booking_data.time, rr_settings['default_datetime_format']).format(format);
        ...
        finalComformation: function (event) {
            ...
            jQuery.get(rr_ajaxurl, options, function (response) {
                ...
                plugin.$element.find('.final').append('<h4>' + rr_settings['trans.done_message'] + '</h4>');
                ...
                if (rr_settings['submit.redirect'] !== '') {
                    setTimeout(function () {
        ...
        triggerEvent: function () {
            ...
            // Define that the event name is 'rangeappnewappointment'.
            event.initEvent('rangeappnewappointment', true, true);
